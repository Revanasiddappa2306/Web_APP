...
const [field_3, setfield_3] = React.useState("");
  ...
  React.useEffect(() => {
    fetch("http://localhost:5000/api/tables/get-table-data", {
      ...
      body: JSON.stringify({ tableName: "Siddu_Malkood_Table" })
    })
      .then(res => res.json())
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 text-gray-900">
      ...
      <main className="flex-1 p-6 flex flex-col items-center justify-start w-full">
        <h1 className="text-2xl font-bold mb-6 text-center">Siddu Malkood</h1>
            ...
            <table className="max-w-full bg-white border border-gray-300 shadow mx-auto" style={{ minWidth: "780px" }}>
              <thead>
                <tr>
                  ...
                  <th className="p-2 border-b min-w-[120px]">Name</th><th className="p-2 border-b min-w-[120px]">Dept</th><th className="p-2 border-b min-w-[120px]">Date</th><th className="p-2 border-b min-w-[120px]">Shift</th>
                </tr>
              </thead>
              <tbody>
                {filteredTableData.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="p-2 border-b min-w-[120px]">{row["Name"]}</td><td className="p-2 border-b min-w-[120px]">{row["Dept"]}</td><td className="p-2 border-b min-w-[120px]">{row["Date"] ? new Date(row["Date"]).toLocaleDateString() : ""}</td><td className="p-2 border-b min-w-[120px]">{row["Shift"]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
