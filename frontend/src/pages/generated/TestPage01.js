...
const [field_4, setfield_4] = React.useState("");
  ...
  const handleRowClick = idx => {
    ...
    setfield_0(row["Name"] ?? "");
    setfield_1(row["Email"] ?? "");
    setfield_2(row["Department"] ?? "");
    setfield_3(row["Shift"] ?? "");
    setfield_4(row["Date"] ?? "");
    setEditingIndex(idx);
  };
  ...
  <div className="w-full overflow-x-auto">
    <table className="max-w-full bg-white border border-gray-300 shadow" style={{ minWidth: "960px" }}>
      <thead>
        <tr>
          <th className="p-2 border-b min-w-[60px]"></th>
          <th className="p-2 border-b min-w-[180px]">Name</th><th className="p-2 border-b min-w-[180px]">Email</th><th className="p-2 border-b min-w-[180px]">Department</th><th className="p-2 border-b min-w-[180px]">Shift</th><th className="p-2 border-b min-w-[180px]">Date</th>
        </tr>
      </thead>
      <tbody>
        {filteredTableData.map((row, idx) => (
          ...
          >
            ...
            <td className="p-2 border-b min-w-[180px]">{row["Name"]}</td><td className="p-2 border-b min-w-[180px]">{row["Email"]}</td><td className="p-2 border-b min-w-[180px]">{row["Department"]}</td><td className="p-2 border-b min-w-[180px]">{row["Shift"]}</td><td className="p-2 border-b min-w-[180px]">{row["Date"] ? new Date(row["Date"]).toLocaleDateString() : ""}</td>
          </tr>
        ))}
      </tbody>
    </table>
