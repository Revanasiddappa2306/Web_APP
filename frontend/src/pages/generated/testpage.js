...
const [field_4, setfield_4] = React.useState("");
  ...
  React.useEffect(() => {
    fetch("http://localhost:5000/api/tables/get-table-data", {
      ...
      body: JSON.stringify({ tableName: "testpage_Table" })
    })
      .then(res => res.json())
  ...
  const handleRowClick = idx => {
    ...
    setfield_2(row["name"] ?? "");
    setfield_3(row["HeadCount"] ?? "");
    setfield_4(row["Phone"] ?? "");
    setEditingIndex(idx);
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 text-gray-900">
      ...
      <main className="flex-1 p-6 flex flex-col items-center justify-start w-full">
        <h1 className="text-2xl font-bold mb-6 text-center">testpage</h1>
            ...
            <table className="table-auto bg-white border border-gray-300 shadow mx-auto">
              <thead>
                <tr>
                  ...
                  <th className="p-2 border-b border-r min-w-[120px] w-[120px]">Select Date</th><th className="p-2 border-b border-r min-w-[160px] w-[160px]">Department</th><th className="p-2 border-b border-r min-w-[160px] w-[160px]">name</th><th className="p-2 border-b border-r min-w-[80px] w-[80px]">HeadCount</th><th className="p-2 border-b border-r min-w-[80px] w-[80px]">Phone</th>
                </tr>
              </thead>
              <tbody>
                {filteredTableData.map((row, idx) => (
                  ...
                  >
                    ...
                    <td className="p-2 border-b border-r min-w-[120px] w-[120px]">{row["Select_Date"] ? new Date(row["Select_Date"]).toLocaleDateString() : ""}</td><td className="p-2 border-b border-r min-w-[160px] w-[160px]">{row["Department"]}</td><td className="p-2 border-b border-r min-w-[160px] w-[160px]">{row["name"]}</td><td className="p-2 border-b border-r min-w-[80px] w-[80px]">{row["HeadCount"]}</td><td className="p-2 border-b border-r min-w-[80px] w-[80px]">{row["Phone"]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
